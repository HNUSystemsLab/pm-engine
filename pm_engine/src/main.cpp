...
namespace storage {
  ...
  static void parse_arguments(int argc, char* argv[], config& state) {
    ...
    state.fs_path = std::string("/mnt/pmem/");
    state.num_keys = 10;
    state.num_txns = 10;
    ...
    state.gc_interval = 5;
    state.ycsb_per_update = 0.1;
    state.ycsb_per_delete = 0;
    state.ycsb_per_insert = 0;
    ...
    while (1) {
      int idx = 0;
      int c = getopt_long(argc, argv, "n:f:x:k:e:p:g:q:b:v:j:swascmhludytzori", opts,
                          &idx);
...
fail:
	...
	exit(ret);
      ...
      case 'f':
        ...
        state.ycsb_per_insert = atof(optarg);
        std::cerr <<"per_insert " << state.ycsb_per_insert << std::endl;
        break;
      case 'x':
      ...
      case 'v':
        //state.verbose = true;
        //level = 3;
        state.ycsb_per_delete = atof(optarg);
        std::cerr << "per_delete: "<< state.ycsb_per_delete << std::endl;
        break;
      case 'p':
        state.ycsb_per_update = atof(optarg);
        assert(state.ycsb_per_update >= 0 && state.ycsb_per_update <= 1);

        if (state.ycsb_per_update == 0 && state.ycsb_per_delete == 0 && state.ycsb_per_insert == 0)
          state.read_only = true;
        std::cerr << "per_updates: " << state.ycsb_per_update << std::endl;
        break;
      case 'g':
        state.ycsb_field_size = atoi(optarg);
        //state.gc_interval = atoi(optarg);
        std::cerr << "ycsb kv size: " << state.ycsb_field_size << std::endl;
        break;
      case 'a':
        state.etype = engine_type::WAL;
      ...
      case 'c':
        ...
        break;
      case 'm':
        state.etype = engine_type::NOF_LSM;
        std::cerr << "nof lsm_enable " << std::endl;
        break;
      case 'l':
...
int main(int argc, char **argv) {
  ...
  const char* path = "/mnt/pmem/zfile";
...
// Start
  ...
  if(state.recovery){
    storage::start_lat();
    //execute recovery
    storage::pmemalloc_recover();
    storage::end_lat(OP_RECOVER);
  }
