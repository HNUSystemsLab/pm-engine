...
#define BT_FIXPADDING    0x01   /* internal */
  ...
  int cow_btree_open_fd(int _fd) {
    ...
    if (persist == false) {
      ...
      mpages = new ((ptree<pgno_t, mpage*>*) \
        pmalloc(sizeof(ptree<pgno_t, mpage*>)))ptree<pgno_t, mpage*>(&sp->ptrs[sp->itr++]);
  ...
  //  if ((page_cache = new struct page_cache()) == NULL)
   if ((page_cache = new((struct page_cache*)pmalloc(sizeof(struct page_cache))) struct page_cache()) == NULL)
      goto fail;
    if (persist)
  ...
  //  if ((lru_queue = new struct lru_queue()) == NULL)
   // lru_queue996 = new ((struct lru_queue*) pmalloc(sizeof(lru_queue))) struct lru_queue();
   // if (lru_queue996 == NULL)

   if ((lru_queue = new ((struct lru_queue*)pmalloc(sizeof(struct lru_queue))) struct lru_queue()) == NULL)
      goto fail;
    if (persist)
...
//        path = strdup(_path);
  ...
  //  copy->page = (page*) new char[head.psize];
    ...
    start_lat();
#endif
    copy = new ((mpage*)pmalloc(sizeof(mpage)))mpage();
    copy->page = (page*)(char*)pmalloc(sizeof(char)*head.psize);
#ifdef FUNC
    end_lat(OP_ALC);
#endif
  ...
  //  if ((_txn = new cow_btree_txn()) == NULL) {
   ...
   start_lat();
#endif
   if ((_txn = new ((cow_btree_txn*)pmalloc(sizeof(cow_btree_txn)))cow_btree_txn()) == NULL) {
#ifdef FUNC2
  end_lat(OP_ALC);
#endif
      DPRINTF("new: %s \n", strerror(errno));
    ...
    //  _txn->dirty_queue = new dirty_queue();
     _txn->dirty_queue = new ((dirty_queue*)pmalloc(sizeof(dirty_queue))) dirty_queue();
  ...
  start_lat();
#endif
  if ((p = (page*)  new ((char*)pmalloc(sizeof(char)*psize)) char()) == NULL)
      return -1;
#ifdef FUNC2
  end_lat(OP_ALC);
#endif
    ...
    start_lat();
#endif
    mpage * header=new ((mpage*)pmalloc(sizeof(mpage))) mpage();
#ifdef FUNC2
    end_lat(OP_ALC);
#endif
  ...
  //  if ((ppage = new struct ppage()) == NULL)
   ...
   start_lat();
#endif
   if ((ppage = new ((struct ppage*)pmalloc(sizeof(struct ppage))) struct ppage()) == NULL)
      return NULL;
#ifdef FUNC2
   end_lat(OP_ALC);
#endif    
  ...
  struct mpage * cow_btree_get_mpage(pgno_t pgno) {
    ...
    if (mp == NULL) {
      ...
      //  mp->page = (page*) new char[head.psize];
        mp = new ((mpage*)pmalloc(sizeof(mpage)))mpage();
        mp->page = (page*)((char*)pmalloc(sizeof(char)*head.psize));
  ...
  int cow_btree_read_data(struct mpage *mp, struct cow_node *leaf,
    ...
    if (!F_ISSET(leaf->flags, F_BIGDATA)) {
      ...
      //    if ((data->data = new char[data->size]) == NULL)
        if ((data->data = (char*)pmalloc(sizeof(char)*data->size)) == NULL)
            return BT_FAIL;
   ...
   // if ((data->data = new char[leaf->n_dsize]) == NULL)
    if ((data->data = (char*)pmalloc(sizeof(char)*leaf->n_dsize)) == NULL)
      return BT_FAIL;
  ...
  int bt_set_key(struct mpage *mp, struct cow_node *cow_node,
    ...
    //  key->data = new char[key->size];
     ...
     start_lat();
#endif
     key->data = (char*)pmalloc(sizeof(char)*key->size);
#ifdef FUNC2
     end_lat(OP_ALC);
#endif
  ...
  //  if ((mp = new mpage()) == NULL)
    ...
    start_lat();
#endif
     if ((mp = new((mpage*)pmalloc(sizeof(mpage))) mpage()) == NULL)
      return NULL;
  //  if ((mp->page = (page*) new char[head.psize]) == NULL) {
    if ((mp->page = (page*) ((char*)pmalloc(sizeof(char)*head.psize))) == NULL) {
      delete mp;
      return NULL;
    }
#ifdef FUNC2
  ...
  //  if ((cursor = new struct cursor()) != NULL) {
   ...
   start_lat();
#endif
   if ((cursor = new ((struct cursor*)pmalloc(sizeof(struct cursor)))struct cursor()) != NULL) {
#ifdef FUNC2  
   end_lat(OP_ALC);
#endif      
...
#define FILL_THRESHOLD   250
  ...
  //  if ((copy = (page*) new char[head.psize]) == NULL)
   ...
   start_lat();
#endif
   if ((copy = (page*) ((char*)pmalloc(sizeof(char)*head.psize))) == NULL)
      return BT_FAIL;
#ifdef FUNC2
   end_lat(OP_ALC);
#endif
  ...
  start_lat();
#endif
  if ((p = (page*) ((char*)pmalloc(sizeof(char)*head.psize))) == NULL)
      return P_INVALID;
#ifdef FUNC2
  end_lat(OP_ALC);
#endif   
  ...
  int compact() {
    ...
    if (persist == false) {
      ...
      btc = new ((cow_btree*)pmalloc(sizeof(cow_btree)))  cow_btree(persist, tmp_fd);
 ...
 public:
  cow_pbtree(bool _persist, const char* path, void** _ptr) {
    ...
    if (_persist) {
      ...
      if (t_ptr == NULL) {
        t_ptr = new ((cow_btree*)pmalloc(sizeof(cow_btree)))cow_btree(_persist, (char*) NULL);
    ...
    // File mode
      ...
      t_ptr = new ((cow_btree*)pmalloc(sizeof(cow_btree)))  cow_btree(_persist, path);
