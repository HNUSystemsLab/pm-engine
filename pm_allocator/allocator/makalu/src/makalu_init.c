...
STATIC void* MAK_CALL MAK_set_persistent_rgn(MAK_persistent_memalign memalign_func)
{
    ...
    printf("map_size is %ld\n", MAXOBJBYTES+1);
    printf("header size is %ld\n", MAX_EXTRA_BYTES);
    printf("block size is %ld\n", CPP_HBLKSIZE);
    size_t total_alloc_sz = sizeof (struct _MAK_base_md)// 54464
                 + (MAXOBJKINDS * sizeof (struct obj_kind))//16*32
                 + sizeof (bottom_index)//8256
                 + (TOP_SZ * sizeof (bottom_index*))//8*TOPSZ
                 + (MAP_LEN * sizeof(short)) * (MAXOBJGRANULES+1);
    //total is 145664, this is metadata region
    void* ret = 0;
