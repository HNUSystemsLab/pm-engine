...
MAK_API void MAK_CALL MAK_fragment(void* base_addr, void* curr_addr, int size, _Float64 **obj_count)
{
    MAK_output_hinchar();
    return;
    //printf("mak fragment!\n");
    char* nvm_address = (char*)base_addr;
    //_Float64 large_rate = 0;
    //_Float64 small_rate = 0;
    size_t large_count = 0;
    size_t small_count = 0;
    size_t tb_count = 0; //total alloc block count
    while(nvm_address<=curr_addr)
    {
        hdr* this_header = MAK_find_header((ptr_t)nvm_address);
        if(this_header != NULL && this_header > base_addr)// this is a valid block
        {
            if(this_header->hb_large_block == 1) // this is a large alloc
            {
                size_t lg = ROUNDED_UP_GRANULES(this_header->hb_sz);
                size_t lb_rounded = GRANULES_TO_BYTES(lg);
                word n_blocks = OBJ_SZ_TO_BLOCKS(lb_rounded);
                tb_count += n_blocks;
                _Float64 this_used_rate = (_Float64) this_header->hb_sz / ((_Float64)n_blocks * (_Float64)HBLKSIZE);
                large_count += n_blocks;
                obj_count[0][size-1] += this_used_rate * n_blocks; //actual used blocks
                obj_count[1][size-1] += n_blocks; //allocator alloc blocks
                //large_rate += this_used_rate  * (_Float64)n_blocks;
            }
            else if(this_header->hb_flags != FREE_BLK) //this is a small alloc
            {         
                size_t this_size = this_header->hb_sz;
                int pos = BYTES_TO_GRANULES(this_size);
                size_t this_total = this_header->hb_sz * this_header->hb_n_marks; //total bytes this blk used
                //small_rate +=  ((_Float64)this_total / (_Float64) HBLKSIZE);
                obj_count[0][pos]+= ((_Float64)this_total / (_Float64) HBLKSIZE); //actual used blocks
                obj_count[1][pos]++; //allocator alloc blocks
                tb_count++;
                small_count++;
            }
            else
        ...
        nvm_address += HBLKSIZE; // this is probably a header region or partical large alloc, skip it
    }
    printf("total alloc is %ld, large alloc used %ld block, small alloc used %ld block\n", tb_count, large_count, small_count);
    //printf("precise: small_rate is %f, large_rate is %f\n", small_rate, large_rate);
}
