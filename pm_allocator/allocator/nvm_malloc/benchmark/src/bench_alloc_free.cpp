...
void worker(int id) {
  ...
  std::uniform_int_distribution<uint64_t> distribution(allocation_size_min,
                                                       allocation_size_max);
...
int main(int argc, char** argv) {
  if (argc < 3 || argc > 4) {
    std::cout << "usage: " << argv[0]
              << " <num_threads> <allocation_size_min> [allocation_size_max]"
              << std::endl;
    return -1;
  }
  ...
  if (allocation_size_min < 64) {
    std::cout << "WARNING: specified min allocation size was less than "
  ...
  if (argc == 4) {
    ...
    if (allocation_size_max < allocation_size_min) {
      std::cout
          << "WARNING: max allocation size was less than min, using min instead"
          << std::endl;
