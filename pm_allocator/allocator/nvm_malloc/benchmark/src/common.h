...
struct StringHashCompare {
  static size_t hash(const std::string x) {
    size_t h = 0;
    for (const char *s = x.c_str(); *s; ++s) h = (h * 17) ^ *s;
    return h;
  }
  static bool equal(const std::string x, const std::string y) { return x == y; }
};
typedef tbb::concurrent_hash_map<std::string, void *, StringHashCompare>
    object_table_t;
extern object_table_t _object_table;
#endif

inline void *initialize(const std::string workspace_path,
                        int recover_if_possible) {
#ifdef USE_MALLOC
  return nullptr;
#elif USE_NVM_MALLOC
...
inline void activate(void *ptr, void **link_ptr1 = nullptr,
                     void *target1 = nullptr, void **link_ptr2 = nullptr,
                     void *target2 = nullptr) {
#ifdef USE_MALLOC
  if (link_ptr1) {
    *link_ptr1 = target1;
    if (link_ptr2) {
...
inline void free(void *ptr, void **link_ptr1 = nullptr, void *target1 = nullptr,
                 void **link_ptr2 = nullptr, void *target2 = nullptr) {
#ifdef USE_MALLOC
  ::free(ptr);
  if (link_ptr1) {
    *link_ptr1 = target1;
...
struct timer {
  ...
  inline void start() { gettimeofday(&t_start, nullptr); }
  inline uint64_t stop() {
    gettimeofday(&t_end, nullptr);
    return ((uint64_t)t_end.tv_sec - t_start.tv_sec) * 1000000ul +
