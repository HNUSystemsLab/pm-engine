...
void *meta_info =
    NULL; /* must be non-static as it is exported to other translation units */
...
void *initalize_nvm_space(const char *workspace_path, uint64_t max_num_chunks) {
  ...
  // perform initial request for large memory block, CHUNK_SIZE *must* be a
  // multiple of 2mb
  if ((chunk_region_start = mmap(NULL, max_chunks * CHUNK_SIZE, PROT_NONE,
                                 MAP_SHARED | MAP_ANONYMOUS | MAP_NORESERVE, -1,
                                 0)) == MAP_FAILED) {
  ...
  backing_file_path =
      (char *)malloc(base_path_length + 1 + 7 +
                     1); /* <workspace_path> + '/' + 'backing' + '\0' */
  meta_file_path =
      (char *)malloc(base_path_length + 1 + 4 +
                     1); /* <workspace_path> + '/' + 'meta' + '\0' */
...
void initialize_chunks() {
  ...
  /* >>>> HACK begin: call nvm_fallocate with 1MB first to prevent PMFS from
   * switching to huge pages */
  ...
  if ((meta_info = mmap(NULL, BLOCK_SIZE, PROT_READ | PROT_WRITE,
                        MAP_SHARED | MAP_NORESERVE, meta_file_fd, 0)) ==
      MAP_FAILED)
...
uint64_t recover_chunks() {
  void *next_chunk_addr =
      (void *)((char *)chunk_region_start + next_chunk * CHUNK_SIZE);
  ...
  if (mmap(next_chunk_addr, n_bytes, PROT_READ | PROT_WRITE,
           MAP_SHARED | MAP_NORESERVE | MAP_FIXED, backing_file_fd,
           0) == MAP_FAILED) {
...
void *activate_more_chunks(uint64_t n_chunks) {
  ...
  next_chunk_addr =
      (void *)((uintptr_t)chunk_region_start + next_chunk * CHUNK_SIZE);

  if (nvm_fallocate(backing_file_fd, next_chunk * CHUNK_SIZE,
                    n_chunks * CHUNK_SIZE) != 0)
    error_and_exit("unable to increase file size of %s", backing_file_path);
  if (mmap(next_chunk_addr, n_chunks * CHUNK_SIZE, PROT_READ | PROT_WRITE,
           MAP_SHARED | MAP_NORESERVE | MAP_FIXED, backing_file_fd,
           next_chunk * CHUNK_SIZE) == MAP_FAILED)
...
uint64_t get_next_chunk()
{
  uint64_t temp = next_chunk;
  return temp;
}
