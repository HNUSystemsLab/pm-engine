...
void* nvm_initialize(const char *workspace_path, int recover_if_possible) {
    ...
    int how_small = SCLASS_SMALL_MAX;
    int how_big = SCLASS_LARGE_MIN;
    int small_small = SCLASS_SMALL_MIN;
    int big_big = SCLASS_LARGE_MAX;
    printf("small max is %d, small min is %d\n",how_small, small_small);
    printf("big_min is %d, big_big is %d\n", how_big, big_big);
    if (nvm_start != NULL) {
        return nvm_start;
    }
    nvm_start = initalize_nvm_space(workspace_path, MAX_NVM_CHUNKS);//100G/4M
...
int find(uint32_t* ptr, int size, uint32_t key)
{
...
void nvm_get_fragment(int size, int count, double **objsize)
{
    //pid_t tid;
    //arena_t *this_arena;
    //tid = (uint64_t) syscall(SYS_gettid);
    //chainhash_itr_t(tmap) iter = chainhash_get(tmap, tidmap, tid);
    //this_arena = chainhash_value(tmap, iter);
    //3 level of alloc
    //output_counter();
    //return;
    nvm_run_header_t *nvm_run;
    nvm_block_header_t *nvm_block;
    nvm_huge_header_t *nvm_huge;

    uint64_t small_alloc_count=0;//total used block of small alloc
    uint64_t large_alloc_count=0;//total used block of large alloc
    char * address_block = (char*) nvm_start;
    //address_block += 0x1000;// first block in this chunk is used for alloc by id, skip it
    char * end_address = (char*)((uintptr_t)nvm_start + get_next_chunk() * CHUNK_SIZE);
    uint64_t total_block_num = get_next_chunk() * 1024;
    while(address_block<end_address) //scan the whole heap
    {
        for(int i=1023; i>=0; i--)
        {
            nvm_run = address_block + i * BLOCK_SIZE; //start from first used block in this chunk
            nvm_block = (nvm_block_header_t*)nvm_run; //one block may be used to small and large alloc, check both
            if(i==0) //chunk header only apper at a chunk's start, and it takes the whole block
            {
                nvm_huge = (nvm_huge_header_t*)nvm_run;
                if(GET_USAGE(nvm_huge->state) == USAGE_HUGE) // this is a huge alloc
                {
                    //TODO: add huge alloc fragment code
                }    
            }
            else
            {
                if(GET_USAGE(nvm_run->state) == USAGE_RUN) //this is a small alloc
                {
                    if(size == -1 && nvm_run->version != 1) //test for recover vheader is unreachable sometimes
                    {
                        continue;
                    }
                    else
                    {
                        arena_run_t *start_run = nvm_run->vdata;
                        uint32_t small_aligned_size = start_run->elem_size;
                        int pos = small_aligned_size / 64 - 1; //bins start from 0,
                        int run_used_count = start_run->n_max - start_run->n_free;
                        if (start_run->n_free==0) //total block is used
                            objsize[0][pos]++;
                        else
                        {
                            double used_rate = (double)run_used_count / (double)start_run->n_max; //get exact used rate
                            objsize[0][pos]+= used_rate;
                        }
                        small_alloc_count++;    
                        objsize[1][pos]++;
                    }
                }
                if(GET_USAGE(nvm_block->state) == USAGE_BLOCK)//this is a large alloc
                {
                    uint32_t large_aligned_size = nvm_block->n_pages * BLOCK_SIZE;
                    large_alloc_count += nvm_block->n_pages;
                    objsize[1][31] += nvm_block->n_pages;
                    objsize[0][31] += nvm_block->n_pages;
                }
                if(GET_USAGE(nvm_block->state) == USAGE_HUGE)
                {
                    uint32_t large_aligned_size = nvm_block->n_pages * BLOCK_SIZE;
                    large_alloc_count += nvm_block->n_pages;
                    objsize[1][31] += nvm_block->n_pages;
        ...
        address_block += 0x400000; //done with one chunk, block ptr move on to next chunk start
    }
    printf("total alloc block is %ld, small alloc used %ld block, large alloc used %ld block\n", 
        total_block_num, small_alloc_count, large_alloc_count);
    //TODO: use the obj_used_array to get more precise fragment data
